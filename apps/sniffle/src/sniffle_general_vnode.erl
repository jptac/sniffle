...
-spec handle_command(term(), any(), any()) ->
                            {reply, any(), any()} |
                            {noreply, any()}.
...
handle_command(#req{id = ID, request = {add, Key, Fun}, bucket = Bucket},
               _Sender, State) ->
    Obj = Fun(fifo_db:get(State#state.db, Bucket, Key)),
